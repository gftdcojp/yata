//! yata-cypher transport benchmark: UCX tier simulation vs Neo4j Bolt
//!
...
//!   Tier 4: Neo4j Bolt/TCP      = real Neo4j over TCP loopback (reference)
//!
...
fn generate(n_nodes: usize, n_edges: usize, seed: u64) -> Dataset {
    ...
    let edges = (0..n_edges)
        .map(|i| {
            ...
            (format!("edge_{i}"), format!("node_{src}"), format!("node_{dst}"), rel_types[i % rel_types.len()].to_owned())
        })
        .collect();
    Dataset { nodes, edges }
}
...
fn build_graph(ds: &Dataset) -> MemoryGraph {
    ...
    for (id, src, dst, rel_type) in &ds.edges {
        g.add_rel(RelRef { id: id.clone(), rel_type: rel_type.clone(), src: src.clone(), dst: dst.clone(), props: Default::default() });
    }
    g
}
...
impl Latencies {
    ...
    fn report(&mut self, tier: &str, scenario: &str) -> f64 {
        ...
        let n = self.0.len();
        if n == 0 { println!("  [{tier:<18}] {scenario:<38} no data"); return 0.0; }
            ...
            loop {
                match read_query(&mut stream).await {
                    ...
                    Ok(q) => {
                        ...
                        if send_response(&mut stream, &r).await.is_err() { break; }
                    }
                    Err(_) => break,
                }
            }
...
async fn bench_neo4j(uri: &str, user: &str, pass: &str, ds: &Dataset, queries: &[&str], iters: usize) -> Option<Vec<f64>> {
    let neo = match neo4rs::Graph::new(uri, user, pass).await {
        Ok(g) => g,
        Err(e) => { eprintln!("  [Neo4j] connect failed: {e}"); return None; }
    };

    // load data
    let _ = neo.run(neo4rs::query("MATCH (n) DETACH DELETE n")).await;
    for (id, label, name, age) in &ds.nodes {
        let q = format!("CREATE (:{label} {{id: '{id}', name: '{name}', age: {age}}})");
        let _ = neo.run(neo4rs::query(&q)).await;
    }
    for (_, src, dst, rel_type) in &ds.edges {
        let q = format!("MATCH (a {{id: '{src}'}}), (b {{id: '{dst}'}}) CREATE (a)-[:{rel_type}]->(b)");
        let _ = neo.run(neo4rs::query(&q)).await;
    }
...
async fn main() -> Result<()> {
    ...
    // ── Tier 4: Neo4j Bolt
    println!("─── Tier 4: Neo4j 5 Bolt/TCP (reference) ─────────────────────────────────────");
    println!("  [loading Neo4j dataset...]");
    let t4 = bench_neo4j(&neo4j_uri, &neo4j_user, &neo4j_pass, &ds, queries, iters).await;
    println!();
    ...
    println!("═══════════════════════════════════════════════════════════════════════════════");
    println!("  {:<40}  {:>10}  {:>12}  {:>10}  {:>10}  {:>12}",
        "query", "inprocess", "shmem(UCX1)", "UDS(UCX2)", "TCP(UCX3)", "Neo4j-Bolt");
    println!("  {}", "─".repeat(100));
    ...
    for (i, &q) in queries.iter().enumerate() {
        ...
        let n4  = t4.as_ref().and_then(|v| v.get(i).copied());

        let sh_ov = sh - ip;
        let ud_ov = ud - ip;
        let tc_ov = tc - ip;
        let n4_str = n4.map(|v| format!("{:>8.1}", v)).unwrap_or_else(|| "       N/A".into());

        // short label
        let label = if q.len() > 40 { &q[..40] } else { q };
        println!("  {label:<40}  {ip:>8.1}µs  {sh:>8.1}µs(+{sh_ov:.0})  {ud:>6.1}µs(+{ud_ov:.0})  {tc:>6.1}µs(+{tc_ov:.0})  {n4_str}µs");
    }
    ...
    println!("    Tier 1  UCX-shmem     : {avg_sh:>8.1}µs  (+{:.1}µs  = shm copy + atomic RTT)", avg_sh - avg_ip);
    println!("    Tier 2  UCX-UDS       : {avg_ud:>8.1}µs  (+{:.1}µs  = syscall + kernel copy)", avg_ud - avg_ip);
    println!("    Tier 3  UCX-TCP       : {avg_tc:>8.1}µs  (+{:.1}µs  = TCP stack + Nagle off)", avg_tc - avg_ip);
    if let Some(ref v) = t4 {
        let avg_n4 = v.iter().sum::<f64>() / n;
        println!("    Tier 4  Neo4j-Bolt    : {avg_n4:>8.1}µs  (+{:.1}µs  = Bolt framing + TCP)", avg_n4 - avg_ip);
        println!();
        println!("  Key insight: yata-cypher UCX-TCP vs Neo4j-Bolt:");
        println!("    Both use TCP loopback — difference = Bolt protocol overhead + Neo4j executor");
        println!("    yata-cypher TCP mean: {avg_tc:.1}µs  vs  Neo4j Bolt mean: {avg_n4:.1}µs  → {:.1}x faster", avg_n4 / avg_tc);
    }
    ...
    for &sz in &[100usize, 500, 1_000, 5_000, 10_000] {
        ...
        for _ in 0..10 {
            ...
            shm_req_len(ptr).store(qlen as u32, Ordering::Release);
            shm_state(ptr).store(1, Ordering::Release);
            loop { if shm_state(ptr).load(Ordering::Acquire) == 2 { break; } std::hint::spin_loop(); }
            shm_state(ptr).store(0, Ordering::Release);
        }
        ...
        let ops  = 1_000_000.0 / mean;
        println!("  nodes={sz:>6}  edges={:>6}  mean={mean:>7.1}µs  p99={p99:>7}µs  {ops:>8.0} ops/s", sz * 3);

        // shutdown
        let qb2 = b"__quit__";
        unsafe { std::ptr::copy_nonoverlapping(qb2.as_ptr(), ptr.add(OFF_REQ_BUF), qb2.len()); }
