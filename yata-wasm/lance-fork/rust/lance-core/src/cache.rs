...
mod wasm_cache {
    ...
    pub enum CapacityMode { Items, Bytes }
    ...
    impl FileMetadataCache {
        ...
        pub fn with_capacity(_capacity: usize, _mode: CapacityMode) -> Self { Self }
        pub fn with_base_path(&self, _base_path: Path) -> Self { Self }
        pub fn size(&self) -> usize { 0 }
        pub fn approx_size(&self) -> usize { 0 }
        pub fn get_by_str<T: Send + Sync + 'static>(&self, _: &str) -> Option<Arc<T>> { None }
        pub fn get<T: Send + Sync + 'static>(&self, _: &Path) -> Option<Arc<T>> { None }
        pub fn insert<T: DeepSizeOf + Send + Sync + 'static>(&self, _: Path, _: Arc<T>) {}
        pub fn insert_by_str<T: DeepSizeOf + Send + Sync + 'static>(&self, _: &str, _: Arc<T>) {}
        pub async fn get_or_insert<T: DeepSizeOf + Send + Sync + 'static, F, Fut>(&self, path: &Path, loader: F) -> Result<Arc<T>>
        where F: Fn(&Path) -> Fut, Fut: Future<Output = Result<T>> {
