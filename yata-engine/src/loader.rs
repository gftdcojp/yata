...
/// Load all vertices/edges from Lance into a fresh MutableCsrStore.
pub async fn load_csr_from_lance(store: &LanceGraphStore) -> Result<MutableCsrStore, String> {
    ...
    tracing::info!(
        ...
        edges = edges.len(),
        "engine: CSR initialized from Lance"
    );
    Ok(csr)
}
