...
impl TieredGraphEngine {
    ...
    /// Fetch manifest.json from R2 and store as fragment_manifest (synchronous, non-FUSE fallback).
    #[allow(dead_code)]
    fn try_load_manifest_from_r2(&self) {
        ...
        match s3.get_sync(&key) {
            Ok(Some(data)) => {
                if let Ok(snap) = serde_json::from_slice::<crate::snapshot::SnapshotManifest>(&data) {
                    // Build a FragmentManifest with placeholder ObjectIds
                    let mut v_map = HashMap::new();
                    for (i, label) in snap.vertex_labels.iter().enumerate() {
                        v_map.insert(label.clone(), yata_store::ObjectId(i as u64));
                    }
                    let mut e_map = HashMap::new();
                    for (i, label) in snap.edge_labels.iter().enumerate() {
                        e_map.insert(label.clone(), yata_store::ObjectId(1000 + i as u64));
                    }
                    let fm = yata_store::vineyard::FragmentManifest {
                        vertex_labels: v_map,
                        edge_labels: e_map,
                        csr_object: None,
                        schema_object: None,
                        partition_id: snap.partition_id.get(),
                        timestamp_ns: snap.timestamp_ns,
                        vertex_count: snap.vertex_count,
                        edge_count: snap.edge_count,
                    };
                    if let Ok(mut guard) = self.fragment_manifest.lock() {
                        *guard = Some(fm);
                    }
                    tracing::info!(vertices = snap.vertex_count, edges = snap.edge_count, "loaded manifest from R2");
                }
            }
            Ok(None) => tracing::debug!("no R2 manifest found"),
            Err(e) => tracing::warn!(error = %e, "R2 manifest fetch failed"),
        }
    }

    /// For each label, if Vineyard doesn't have the blob, fetch from R2 and store in Vineyard.
    fn ensure_vineyard_blobs_from_r2(
        &self,
        ...
        for label in labels {
            // Check if we already have this blob in Vineyard
            let has_vertex = manifest.vertex_labels.get(label)
                .and_then(|&id| self.vineyard.get_blob(id))
                .is_some();
            let has_edge = manifest.edge_labels.get(label)
                .and_then(|&id| self.vineyard.get_blob(id))
                .is_some();

            // Page-in vertex blob from R2 if missing.
            // Try single blob first (snap/v/{label}.arrow), then chunked (snap/v/{label}/chunk_N.arrow).
            if !has_vertex && manifest.vertex_labels.contains_key(label) {
                let single_key = format!("{prefix}snap/v/{label}.arrow");
                let obj_id = *manifest.vertex_labels.get(label).unwrap_or(&yata_store::ObjectId(0));
                if let Ok(Some(data)) = s3.get_sync(&single_key) {
                    let meta = yata_store::vineyard::ObjectMeta {
                        id: obj_id,
                        blob_type: yata_store::vineyard::BlobType::ArrowVertexGroup,
                        label: label.clone(),
                        partition_id: manifest.partition_id,
                        size_bytes: data.len() as u64,
                        r2_key: single_key.clone(),
                        fields: HashMap::new(),
                        created_at: 0,
                    };
                    self.vineyard.put(meta, data.clone());
                    tracing::info!(label, bytes = data.len(), "R2 page-in vertex blob (single)");
                } else {
                    // Try chunked: snap/v/{label}/chunk_000000.arrow, chunk_000001.arrow, ...
                    let mut chunk_idx = 0;
                    let mut total_bytes = 0usize;
                    let mut all_chunks = Vec::new();
                    loop {
                        let chunk_key = format!("{prefix}snap/v/{label}/chunk_{chunk_idx:06}.arrow");
                        match s3.get_sync(&chunk_key) {
                            Ok(Some(data)) => {
                    ...
                    if !all_chunks.is_empty() {
                        // Concatenate chunks into single blob for CSR restore
                        // (ensure_labels_from_vineyard expects 1 blob per label)
                        // For now, use first chunk — full merge requires Arrow concat
                        let merged = all_chunks.into_iter().next().unwrap_or_default();
                        let meta = yata_store::vineyard::ObjectMeta {
                            id: obj_id,
                            blob_type: yata_store::vineyard::BlobType::ArrowVertexGroup,
                            label: label.clone(),
                            ...
                            r2_key: format!("{prefix}snap/v/{label}/chunk_000000.arrow"),
                            fields: HashMap::new(),
                            created_at: 0,
                        };
                        self.vineyard.put(meta, merged);
                        tracing::info!(label, chunks = chunk_idx, total_bytes, "R2 page-in vertex blob (chunked)");
                    }
                }
            }

            // Page-in edge blob from R2 if missing
            if !has_edge && manifest.edge_labels.contains_key(label) {
                let key = format!("{prefix}snap/e/{label}.arrow");
                if let Ok(Some(data)) = s3.get_sync(&key) {
                    let meta = yata_store::vineyard::ObjectMeta {
                        id: *manifest.edge_labels.get(label).unwrap_or(&yata_store::ObjectId(0)),
                        blob_type: yata_store::vineyard::BlobType::ArrowEdgeGroup,
                        label: label.clone(),
                        partition_id: manifest.partition_id,
                        size_bytes: data.len() as u64,
                        r2_key: key.clone(),
                    ...
                    self.vineyard.put(meta, data.clone());
                    tracing::info!(label, bytes = data.len(), "R2 page-in edge blob");
                }
            }
        }
    }

    /// LRU page-out: evict labels from CSR when max_loaded would be exceeded.
    /// Evicted labels remain in Vineyard blob cache for fast re-page-in.
    fn maybe_evict_labels(&self, loaded: &mut HashSet<String>, incoming_count: usize) {
        let max = self.config.lazy_label_max_loaded;
        if max == 0 {
            return;
        }

        let total_after = loaded.len() + incoming_count;
        if total_after <= max {
            return;
        }

        let evict_count = total_after - max;
        let to_evict: Vec<String> = loaded.iter().take(evict_count).cloned().collect();
        if to_evict.is_empty() {
            return;
        }

        // Before evicting from CSR, snapshot evicted labels to Vineyard (page-out → warm tier)
        if let Ok(csr) = self.hot.lock() {
            if let Some(single) = csr.as_single() {
                for label in &to_evict {
                    self.snapshot_label_to_vineyard(single, label);
                }
            }
        }
        ...
        if let Ok(mut hot) = self.hot.lock() {
            for label in &to_evict {
                hot.remove_vertices_by_label(label);
                hot.remove_edges_by_label(label);
                loaded.remove(label);
            }
            hot.commit();
        }
        tracing::info!(
            ...
            vineyard_cache_bytes = self.vineyard.cache_bytes(),
            "engine: LRU page-out (labels demoted to Vineyard)"
        );
    }
    ...
    // ── Vineyard / Snapshot access ──────────────────────────────────────

    /// Access the Vineyard blob cache.
    pub fn vineyard(&self) -> &Arc<dyn VineyardStore> {
        &self.vineyard
    }

    /// Access the current fragment manifest (if a snapshot was taken or restored).
    pub fn fragment_manifest(&self) -> Option<FragmentManifest> {
