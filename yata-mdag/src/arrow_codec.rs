...
//!
//! Replaces CBOR for data blocks (LabelVertexGroup, LabelEdgeGroup).
//! Metadata blocks (GraphRootBlock, SchemaBlock) remain CBOR.
//!
//! Arrow columnar layout enables:
//! - Zero-copy property scan (vs CBOR full-block deserialize)
...
/// Magic bytes to distinguish Arrow IPC blocks from CBOR blocks.
/// CAS blocks starting with these 4 bytes use Arrow IPC decoding.
pub const ARROW_MAGIC: &[u8; 4] = b"ARW\x01";

// ── Vertex Group ──────────────────────────────────────────────────────

/// Encode a LabelVertexGroup to Arrow IPC bytes.
/// Layout: fixed columns (vid, labels_json) + dynamic property columns.
pub fn encode_vertex_group(group: &LabelVertexGroup) -> Result<Vec<u8>> {
    ...
    // Collect all property keys across vertices (sorted for deterministic CID)
    let mut all_keys: BTreeMap<String, PropType> = BTreeMap::new();
    for v in &group.vertices {
        for (k, val) in &v.props {
            let pt = prop_type(val);
            all_keys.entry(k.clone()).or_insert(pt);
        }
    }

    // Build schema: vid (i32) + labels_json (utf8) + dynamic props
    let mut fields = vec![
    ...
    let batch = RecordBatch::try_new(schema.clone(), columns)
        .map_err(|e| MdagError::Cbor(format!("arrow batch: {e}")))?;

    // Write Arrow IPC stream
    let mut buf = Vec::with_capacity(n * 128);
    buf.extend_from_slice(ARROW_MAGIC);
    let mut writer = StreamWriter::try_new(&mut buf, &schema)
        .map_err(|e| MdagError::Cbor(format!("arrow writer: {e}")))?;
    writer
        .write(&batch)
        .map_err(|e| MdagError::Cbor(format!("arrow write: {e}")))?;
    writer
        .finish()
        .map_err(|e| MdagError::Cbor(format!("arrow finish: {e}")))?;
...
pub fn decode_vertex_group(data: &[u8]) -> Result<LabelVertexGroup> {
    if data.len() < 4 || &data[..4] != ARROW_MAGIC {
        return Err(MdagError::Deserialize("not an Arrow IPC block".into()));
    }
    let ipc_data = &data[4..];

    let reader = StreamReader::try_new(std::io::Cursor::new(ipc_data), None)
        .map_err(|e| MdagError::Deserialize(format!("arrow reader: {e}")))?;
    ...
    for batch_result in reader {
        let batch =
            batch_result.map_err(|e| MdagError::Deserialize(format!("arrow batch: {e}")))?;
        let n = batch.num_rows();

        let vid_col = batch
            .column_by_name("_vid")
            .and_then(|c| c.as_any().downcast_ref::<Int32Array>())
            .ok_or_else(|| MdagError::Deserialize("missing _vid column".into()))?;
        let labels_col = batch
            .column_by_name("_labels")
            .and_then(|c| c.as_any().downcast_ref::<StringArray>())
            .ok_or_else(|| MdagError::Deserialize("missing _labels column".into()))?;
        ...
        // Property columns (everything except internal _vid, _labels, _label)
...
pub fn encode_edge_group(group: &LabelEdgeGroup) -> Result<Vec<u8>> {
    ...
    let batch = RecordBatch::try_new(schema.clone(), columns)
        .map_err(|e| MdagError::Cbor(format!("arrow edge batch: {e}")))?;
...
pub fn decode_edge_group(data: &[u8]) -> Result<LabelEdgeGroup> {
    if data.len() < 4 || &data[..4] != ARROW_MAGIC {
        return Err(MdagError::Deserialize("not an Arrow IPC edge block".into()));
    }
...
/// Check if CAS block data is Arrow IPC (vs legacy CBOR).
...
fn encode_empty_vertex_group(label: &str) -> Result<Vec<u8>> {
    ...
    .map_err(|e| MdagError::Cbor(e.to_string()))?;

    let mut buf = Vec::new();
    buf.extend_from_slice(ARROW_MAGIC);
    let mut writer =
        StreamWriter::try_new(&mut buf, &schema).map_err(|e| MdagError::Cbor(e.to_string()))?;
    writer
        .write(&batch)
        .map_err(|e| MdagError::Cbor(e.to_string()))?;
    writer
...
mod tests {
    ...
    fn test_vertex_group_arrow_roundtrip() {
        ...
        // Check props preserved
        let v0 = &decoded.vertices[0];
        assert!(
            v0.props
                .iter()
                .any(|(k, v)| k == "make" && *v == PropValue::Str("Toyota".into()))
        );
        assert!(
            v0.props
                .iter()
                .any(|(k, v)| k == "horsepower" && *v == PropValue::Int(382))
        );
    }
    ...
    fn test_edge_group_arrow_roundtrip() {
        ...
        assert_eq!(decoded.edges[0].dst, 0);
        assert!(
            decoded.edges[0].props.iter().any(|(k, v)| k == "rating"
                && matches!(v, PropValue::Float(f) if (*f - 4.5).abs() < 0.01))
        );
    }
        ...
        let group = LabelVertexGroup {
            ...
            vertices: vec![VertexBlock {
                ...
                props: vec![
                    ...
                    ("b".into(), PropValue::Str("hello".into())),
                ],
            }],
        };
        let b1 = encode_vertex_group(&group).unwrap();
        let b2 = encode_vertex_group(&group).unwrap();
        let cid1 = yata_core::Blake3Hash::of(&b1);
        let cid2 = yata_core::Blake3Hash::of(&b2);
        assert_eq!(
            cid1, cid2,
            "Arrow IPC encoding must be deterministic for CID"
        );
    }
}
